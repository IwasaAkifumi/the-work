...
public class dan : MonoBehaviour {
	...
	// ボムによる攻撃処理
	private void danAttack(){
		Collider[] targets = Physics.OverlapSphere(transform.position , 100f);	// 自分自身を中心に、半径0.7以内にいるColliderを探し、配列に格納.
		foreach(Collider obj in targets){		// targets配列を順番に処理 (その時に仮名をobjとする)
			if(obj.tag == "Enemy"){				// タグ名がEnemyなら
				obj.SendMessage("Damage",danattack);
			}
		}
	}
}